angular.module('admin', ['functions']).controller('adminCtrl', ['$scope', '$http', 'functions', function ($scope, $http, functions) {
    ...
    var getConfig = () => { $http.get('../config/config.json').then(function (response) { $scope.config = response.data; reloadPage(); }); }; getConfig(); $scope.toggleTpl = (x) => { $scope.tpl = x; }; $scope.toggleTpl('login'); var init = () => { $scope.islogin = false; $scope.u = []; sessionStorage.clear(); functions.post('Users', 'Init', {}).then((d) => { $scope.u = d; }); }
    ...
    var signup = (u, accept) => {
        ...
        functions.post('Users', 'Signup', { user: u }).then((d) => { alert(d); });
    }
    ...
    var deleteImg = (x, img) => { functions.post('Products', 'DeleteImg', { productId: x.productId, img: img }).then((d) => { x.gallery = d; }); }
    var newProduct = () => { functions.post('Products', 'Init', {}).then((d) => { $scope.p.push(d); }); }
    var deleteProduct = (x, u) => { functions.post('Products', 'Delete', { productId: x.productId }).then((d) => { getProducts(u.userId); }); }
    $scope.f = { login: (u) => { return login(u); }, logout: () => { return init(); }, signup: (u, accept) => { return signup(u, accept); }, getProducts: (id) => { return getProducts(id); }, save: (x, u) => { return save(x, u) }, upload: (x, idx) => { return upload(x, idx); }, deleteImg: (x, img) => { return deleteImg(x, img); }, newProduct: () => { return newProduct(); }, deleteProduct: (x, u) => { return deleteProduct(x, u); } }
}]);